...
class _suiviState extends State<suivi> {
  ...
  Future<void> _loadSolution() async {
    ...
    try {
      final res = await http.get(Uri.parse('$_baseUrl/solution'));
      if (res.statusCode == 200) {
        final data = jsonDecode(res.body);
        _buildMapObjects(data);
        setState(() => _totalDistance = (data['total_distance_km'] as num).toDouble());
      } else if (res.statusCode == 404) {
        setState(() => _routeError = 'Pas encore de routes optimisées.');
      } else {
        setState(() => _routeError = 'Erreur lors du chargement des routes.');
      }
    } catch (e) {
      setState(() => _routeError = 'Serveur inaccessible.');
    } finally {
